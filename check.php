<?php  
error_reporting(0);
$con = mysqli_connect("localhost", "root", "root", "myweb", "3306");#记得把这个密码进行加密
...
{  
     ...
     echo "<script type='text/javascript'>alert('请填写用户名或邮箱');location='/';</script>";  
...
{  
    ...
    echo "<script type='text/javascript'>alert('请填写密码');location='/';</script>";  
...
{   
    ...
    if((($colum['username'] == $nameoreamil) || ($colum['email'] == $nameoreamil)) && ($colum['password'] == $password)){
		...
		echo"<script type='text/javascript'>alert('登录成功');location='/userinterface.php';</script>"; 
    }     
    else  {
		echo "<script type='text/javascript'>alert('用户名或密码错误！');location='/';</script>"; 
	} 
}  
?>